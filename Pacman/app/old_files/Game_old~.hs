...
-- import Graphics.UI.WXCore (bitmapCreateFromSize, withMemoryDC, selectObject, deselectObject)
...
pacmanGame = do
    ...
    -- paintOn pMain $ \dc view -> drawMaze nodes dc view
    -- mazeBitmap <- bitmapCreate (sz C.screenWidth C.mainPanelHeight)
    -- withBitmapDraw mazeBitmap $ \dc -> drawMaze nodes dc ()
    -- withMemoryDC NullBitmap $ \dc -> do
    --     selectObject dc mazeBitmap
    --     drawMaze nodes dc ()
    --     deselectObject dc mazeBitmap

    mazeBitmap <- bitmapCreate (sz C.screenWidth C.mainPanelHeight)
    dc <- bitmapGetDC mazeBitmap
    drawMaze nodes dc ()
    ...
    let networkDescription :: MomentIO ()
        networkDescription = mdo
            ...
            --     (drawGameState <$> bPacmanPosInt <*> bGhostPosInt <*> bGhostGoal <*> bNodes <*> pure mazeBitmap) <@ etick
            bpaint <- stepper (\_dc _ -> return ()) $
                (drawGameState <$> bPacmanPosInt <*> bGhostPosInt <*> bGhostGoal <*> pure mazeBitmap) <@ etick
...
drawMaze nodes dc _view = do
    drawNodes       nodes       dc
    drawPortalNodes nodes       dc

-- drawGameState :: Point -> Point -> Point -> NodeGroup -> DC a -> b -> IO ()
-- drawGameState :: Point -> Point -> Point -> NodeGroup -> Bitmap -> DC a -> b -> IO ()
-- drawGameState :: Point -> Point -> Point -> DC a -> b -> IO ()
-- drawGameState pacmanPos ghostPos ghostGoal dc _view = do
--     P.drawPacman    pacmanPos   dc
--     G.drawGhost     ghostPos    dc
--     G.drawGhostGoal ghostGoal   dc
drawGameState :: Point -> Point -> Point -> Bitmap () -> DC a -> b -> IO ()
drawGameState pacmanPos ghostPos ghostGoal mazeBitmap dc _view = do
    drawBitmap dc mazeBitmap (pt 0 0) False
    P.drawPacman pacmanPos dc
    G.drawGhost ghostPos dc
    G.drawGhostGoal ghostGoal dc
...
getIntPositionFromState (Vec x y ,_ ,_ ,_) = point (round x) (round y)
    ...
    -- drawPortalNodes nodes       dc
-- TODO: delete
--             reactimate (fmap (putStrLn . ("eUserMove: " ++) . show) eUserMove)
-- reactimate $ (\t -> putStrLn $ "bTimer = " ++ show t) <$> (bTimer <@ etick)
    -- print $ startKey
    -- print $ nodesMap
            -- reactimate $ (\ev -> putStrLn $ "Mouse event: " ++ show ev) <$> eMouse
            -- reactimate $ (\(pt,_) -> putStrLn $ "Left click at " ++ show pt)  <$> eLeftClick
                -- bCurrentNode    = (\(_,currentNode,_,_)         -> currentNode)      <$> bPacmanState
                -- bTargetNode     = (\(_,_,targetNode,_)          -> targetNode)       <$> bPacmanState
                -- bCurrentDir     = (\(_,_,_,currentDirection)    -> currentDirection) <$> bPacmanState
            -- reactimate $ (\pos -> putStrLn $ "ticpos = " ++ show pos) <$> (bPosFloat <@ etick)
